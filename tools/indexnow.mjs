 ...
 * nothing, and refused that key with 403 on every submission after. Five
 ...
 *
 * THE KEY IN THIS TREE IS THE THIRD ONE
 *
 * All of the above landed once, went green on its own push and the next, and
 * was then reverted along with everything else that session had done. That put
 * the refused key back in indexnow.txt and the unwaiting tool back in tools/,
 * so every deploy after it was red again for exactly that reason — four of
 * them before anybody went looking, each answering 403 for a hundred and one
 * addresses.
 *
 * So it is back, and with a key minted afresh rather than the second one. The
 * second was correct while it was served, but the revert took it off the site
 * for the best part of a day while Bing went on being told to look for it,
 * which is the same way the first one died. A key is cheap and being wrong
 * about one costs a day, so the rule is simply this: a key that has ever been
 * submitted while the site was not serving it does not get a second chance.
 *
