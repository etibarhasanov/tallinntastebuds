 ...
 * db/google-places.sql, which is what actually loads them.
 *
 *   node tools/googleplaces.mjs           rewrite db/google-places.sql
 *   node tools/googleplaces.mjs --check   report that it is stale, exit 1
 *
...
const OUT = join(ROOT, 'db', 'google-places.sql');
...
export function build() {
  ...
  out.push('--');
  out.push('-- GENERATED by tools/googleplaces.mjs from exports/tallinn_restaurants.csv.');
  ...
  out.push('--');
  out.push('--   wrangler d1 execute tallinntastebuds         --remote --file=db/google-places.sql');
  out.push('--   wrangler d1 execute tallinntastebuds-preview --remote --file=db/google-places.sql');
  out.push('--');
  ...
  out.push(`UPDATE google_places SET missing_since = ${NOW} WHERE missing_since IS NULL;`);
  out.push('');
  ...
  for (let i = 0; i < places.length; i += BATCH) {
    ...
    out.push(
      ...
      `INSERT INTO google_places (${cols.join(', ')}, first_seen_at, synced_at)\nVALUES\n` +
      rows.join(',\n') + '\n' +
  ...
  for (const m of matched) {
    ...
    out.push(
      `-- ${m.name}\n` +
      `UPDATE google_places SET map_id = ${q(m.map_id)} ` +
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  if (check) {
    if (!stale()) {
      console.log('OK — db/google-places.sql matches the export beside it.');
      process.exit(0);
    }
    console.log('  FAIL  db/google-places.sql is not what tools/googleplaces.mjs would write.');
    console.log('\nRun `node tools/googleplaces.mjs` and commit the result.');
    process.exit(1);
  }
  ...
  console.log(
    `db/google-places.sql — ${result.places.length} places, ` +
    `${result.matched.length} matched to the map.`
  );
  console.log('\nApply it with:');
  console.log('  wrangler d1 execute tallinntastebuds         --remote --file=db/google-places.sql');
  console.log('  wrangler d1 execute tallinntastebuds-preview --remote --file=db/google-places.sql');
}
