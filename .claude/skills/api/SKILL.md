...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | optional, and both or neither. Without a *usable* pair `/api/account` answers `google: false`, every sheet draws the username and password alone, and `/api/google` sends a hand-typed request home. Nothing else changes. Usable is more than present: `googleReady()` trims both and requires the id to end in `.apps.googleusercontent.com`, so a pasted newline or the secret in the id's box reads as absent rather than as a button that ends on Google's `invalid_client` page |
