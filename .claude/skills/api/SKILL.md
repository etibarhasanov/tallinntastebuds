...
| `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` | optional, and both or neither. Without a *usable* pair `/api/account` answers `google: false`, every sheet draws the username and password alone, and `/api/google` sends a hand-typed request home. Nothing else changes. Usable is more than present: `googleReady()` trims both, requires the id to end in `.apps.googleusercontent.com`, and requires every character of it to be a letter, digit, dot, hyphen or underscore — so a pasted newline, a zero-width character trimming cannot reach, or the secret in the id's box reads as absent rather than as a button that ends on Google's `invalid_client` page. Past those, an `invalid_client` is the console and not the binding |
