...
| `GET/POST /api/split` | `api/split.js` | **splitwise** — `split_groups`, `split_members`, `split_expenses`, `split_shares`, `split_settlements`. Every action but `create` and `join` reads the caller's own membership of the group it names first; a non-member is told the group does not exist | `no-store`, for the reason `lists.js` is |
